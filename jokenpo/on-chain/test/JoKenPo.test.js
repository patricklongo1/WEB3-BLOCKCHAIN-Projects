...
contract('MetaCoin', function(accounts) {
  ...
  it("Should play alone", async () => {
    await contract.play(JoKenPo.Options.PAPER, { from: accounts[1], value: web3.utils.toWei("0.01", "ether") })
    const result = await contract.getResult();
    assert(result === "Player 1 choose his/her option. Waiting player 2.", "Error playing alone")
  });

  it("Should play along", async () => {
    await contract.play(JoKenPo.Options.PAPER, { from: accounts[1], value: web3.utils.toWei("0.01", "ether") })
    await contract.play(JoKenPo.Options.ROCK, { from: accounts[2], value: web3.utils.toWei("0.01", "ether") })
    const result = await contract.getResult();
  ...
  it("Should NOT play alone (owner)", async () => {
    try {
      await contract.play(JoKenPo.Options.PAPER, { from: accounts[0], value: web3.utils.toWei("0.01", "ether") })
  ...
  it("Should NOT play alone (invalid option)", async () => {
    try {
      await contract.play(JoKenPo.Options.NONE, { from: accounts[1], value: web3.utils.toWei("0.01", "ether") })
      assert.fail("The play should have thrown an error.")
    } catch (error) {
      assert.include(error.message, "revert", "The play should be reverted.")
    }
  ...
  it("Should NOT play alone (twice in a row)", async () => {
    await contract.play(JoKenPo.Options.PAPER, { from: accounts[1], value: web3.utils.toWei("0.01", "ether") })
    try {
      await contract.play(JoKenPo.Options.ROCK, { from: accounts[1], value: web3.utils.toWei("0.01", "ether") })
      assert.fail("The play should have thrown an error.")
    } catch (error) {
      assert.include(error.message, "revert", "The play should be reverted.")
    }
  ...
  it("Should NOT play alone (invalid bid)", async () => {
    try {
      await contract.play(JoKenPo.Options.ROCK, { from: accounts[1], value: web3.utils.toWei("0.001", "ether") })
      assert.fail("The play should have thrown an error.")
    } catch (error) {
      assert.include(error.message, "revert", "The play should be reverted.")
    }
  ...
  it("Should get leaderBoard", async () => {
    await contract.play(JoKenPo.Options.ROCK, { from: accounts[1], value: web3.utils.toWei("0.01", "ether") })
    await contract.play(JoKenPo.Options.PAPER, { from: accounts[2], value: web3.utils.toWei("0.01", "ether") })
    const winners = await contract.getLeaderBoard();
    assert(winners[0].wallet === accounts[2] && winners[0].wins === "1", "Error getting leaderBoard")
  });
  ...
  it("Should NOT set commission (invalid owner)", async () => {
    try {
      ...
      assert.include(error.message, "revert", "The setCommission should be reverted.")
    }
  });
